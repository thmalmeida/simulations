...
v2_k = round(v2_t*(2^n_bits-1)/(Vdc));		% Voltage signal vector converted to digital value;
...
i1_t = (Vdc*v2_k/(2^n_bits-1) - R2/(R1+R2)*Vdc)*(1/Rb)*(N2/N1);

k1_ = Vdc/(2^n_bits-1)						% Divided parts to find Irms
k2_ = R2/(R1+R2)*Vdc
k3_ = (1/Rb)*(N2/N1)
% Using the peak value;
i1_RMS = max(i1_t)/sqrt(2);					% Current in RMS [A];
